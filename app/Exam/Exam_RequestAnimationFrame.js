...
class BatterMan extends Component {
  constructor(props) {
    super(props)
    this.state = {
      transition: new Morph.Tween(BatmanLogoPaths[0], BatmanLogoPaths[1])
    }
  }
  ...
  nextAnimation = () => {
    ...
    this.setState({
      transition: new Morph.Tween(BatmanLogoPaths[this._current - 1], BatmanLogoPaths[this._current])
    }, () => this.animate(null, this.nextAnimation))
  }
    ...
    return (
      <View style={Styles.wrap}>
        <Surface width={width} height={height}>
          <Shape x={-140} y={100} d={this.state.transition} fill="#000"/>
        </Surface>
      </View>
    )
  }
...
class TransTan extends Component {
  constructor(props) {
    super(props)
    this.state = {
      posY: 0,
  ...
  changePos = () => {
    if (this.state.direction === 0) {
      ...
      GLOBAL.requestAnimationFrame(this._changePos)
    }
  }

  _changePos = () => {
    if (this.state.posY < 100 && this.state.direction === 1) {
      this.state.posY += 1
      this.state.posY === 100 ? this.state.direction = -1 : null
      this._aniView.setNativeProps({style: {transform: [{translateY: this.state.posY}]}})
      GLOBAL.requestAnimationFrame(this._changePos)
    }
    else if (this.state.direction === -1) {
      this.state.posY -= 1
      this.state.posY === 0 ? this.state.direction = 0 : null
      this._aniView.setNativeProps({style: {transform: [{translateY: this.state.posY}]}})
      GLOBAL.requestAnimationFrame(this._changePos)
    }
  }
  ...
  render() {
    return (
      <View style={Styles.wrap}>
        <TouchableOpacity style={Styles.btn} activeOpacity={0.8} onPress={this.changePos}>
          <Text style={{fontSize: 18, color: 'red'}}>{'click me'}</Text>
        </TouchableOpacity>
        <View ref={ref => this._aniView = ref} style={{width, height: 100, backgroundColor: '#feafea'}}/>
      </View>
    )
  }
}

class DotCircle extends Component {
  _aniObj_1 = null
  _syncStartTime = -1//动画开始时间
  _swingDistance = 100//动画按照正弦运动的振幅
  _swingAniTime = 2000//动画运行时间
  _syncDone = false
  ...
  step = () => {
    let p = Math.min(1.0, (Date.now() - this._syncStartTime) / this._swingAniTime);//以时间为step，根据时间推移决定每个step的运动距离
    /**
     * 正弦曲线计算公式：y=Asin(ωx+φ)+k
     * A: 振幅，当物体作轨迹符合正弦曲线的直线往复运动时，其值为行程的1/2
     * ω: 角速度， 控制正弦周期(单位角度内震动的次数)
     * φ: 初相，x=0时的相位；反映在坐标系上则为图像的左右移动
     * k: 偏距，反映在坐标系上则为图像的上移或下移
     * 下例中：Math.PI 反映在坐标系中的横坐标 X 为角度，0  π/2  π  3π/4  2π，如果只需要一个波峰，则横坐标限定为： 0～π
     */
    let tx = (width - 30 - 30) * p;//以p为step，计算每次requestAnimationFrame的时候的当前横坐标, p的范围区间为[0, 1]
    let ty = -(this._swingDistance * Math.sin(Math.PI * p));//计算当前纵坐标
    //执行动画
    this._aniObj_1.setNativeProps({style: {transform: [{translateY: ty}, {translateX: tx}]}});
    //动画循环
    if (p >= 1.0) {
      this._aniObj_1.setNativeProps({style: {transform: [{translateY: 0}, {translateX: 0}]}});
      this._syncStartTime = Date.now();
    }
    //判断退出动画条件
    (!this._syncDone || tx < ((width - 30 - 30))) && GLOBAL.requestAnimationFrame(this.step);//若动画时间小于_swingAniTime动画运行时间，则继续执行requestAnimationFrame
  }

  render() {
    return (<View ref={(ref) => this._aniObj_1 = ref} style={Styles.inner}/>);
  }
}

//export default TransTan
//export default DotCircle
export default BatterMan
